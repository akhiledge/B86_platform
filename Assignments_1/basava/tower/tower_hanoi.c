...
/**@brief : main function to call tower of hanoi function
 * @param : no parameters
...
int main (void) 
{
    ...
    assert (str);                                   /*validation for NULL 
                                                      condition*/
...
/** @brief : tower function to move the disk from source pole to destination 
 *           pole through auxillary pole.
 * @param : source pole, destination pole and aux pole index
 * @return : no return value
 */

void tower (int disk_num, char from_pole, char to_pole, char aux_pole)
{
    if (disk_num == 1){                             /*if disk number is one 
                                                      move it to destination*/
        fprintf (stdout, "disk %d moves from %c to %c\n", 
                           disk_num, from_pole, to_pole);
        return;
    }
    /**@brief : Recursive function call to move the disks from 
