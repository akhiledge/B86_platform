         ...
         argument count of type integer returns void
{
    ...
    if (hasharray[index] == NULL) { ///if index is NULL
        ...
        prev = root;
        while (root != NULL) {
            if (mystrcasecmp(str,root->str) == 0) { ///if root node and \
                next string both are same then increase the count
            ...
            if (mystrcasecmp(str,root->str) == -1) { ///if root string \
                is greater than next string
