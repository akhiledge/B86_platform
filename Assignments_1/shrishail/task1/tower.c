...
int main(void)
{
    ...
    fgets(str, MAX, stdin);
    disk = atoi(str);
    tower_of_honoi(disk, 'A', 'C', 'B'); ///function call for tower of honoi\
    to send disks from source to destination
    free(str);
    str=NULL;
    return 0;
}
void tower_of_honoi(int disk1,
        ...
        char auxilary)
{
    if (disk1 == 1) {///if disk is only one move disk from source to destination
        ...
        call for disks to move from auxilary to destination rod
    }
