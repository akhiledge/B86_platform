...
{
    bst_t *ptr = *root;        /**< ptr is a temporary pointer
				 of type bst_t */
	...
	*root = create_node(*root, value);  /* /Tree empty case */
	...
	prev->left = create_node(prev, value);  /* Function call
						   for create */
	*root = check_parent(root,
		&prev, prev->left);         /* Function call to check parent */
    } else {                           /* Inserting as right node */
	prev->right = create_node(prev, value);
	 *root = check_parent(root,
		&prev, prev->right); /* Function call to check parent */
    }
}
